...
const createIngredient = (request, response) => {
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      response
        .status(201)
        .send(`New ingredient added with ID: ${results.insertId}`);
    }
  );
};
...
const getFavRecipe = (request, response) => {
  ...
  pool.query(
    "SELECT * FROM favoritedRecipes WHERE user_id = $1",
    [user_id],
    (error, results) => {
      if (error) {
        throw error;
...
const createFavRecipe = (request, response) => {
  ...
  pool.query(
    "INSERT INTO favoritedRecipes (user_id, recipe_name, unfavorited) VALUES ($1, $2, $3)",
    ...
    (error, results) => {
      ...
      response
        .status(201)
        .send(`New favorited recipe added with ID: ${results.insertId}`);
    }
  );
};
...
const doTest = async (request, response) => {
  ...
  try {
    ...
    if (!user || user.rows.length === 0) {
      ...
      await pool.query(
        "INSERT INTO USERS (email, auth0_id, username) VALUES ($1, $2, $3)",
        [email, auth0_id, username]
      );
